...
def fetch_sp500_data():
    ...
    try:
        ...
        table = soup.find('table', {'class': 'wikitable'}) or soup.find('table', {'id': 'constituents'}) or soup.find('table')
        
        if table is None:
            st.error("Could not find S&P 500 table on Wikipedia. Using fallback data.")
            # Fallback data with some popular S&P 500 stocks
            return pd.DataFrame([
        ...
        if not sp500_data:
            st.warning("No S&P 500 data found. Using fallback data.")
            return pd.DataFrame([
                {'Symbol': 'AAPL', 'Name': 'Apple Inc.', 'Sector': 'Information Technology'},
                {'Symbol': 'MSFT', 'Name': 'Microsoft Corporation', 'Sector': 'Information Technology'},
                {'Symbol': 'GOOGL', 'Name': 'Alphabet Inc.', 'Sector': 'Communication Services'},
        ...
        st.error(f"Error fetching S&P 500 data: {str(e)}. Using fallback data.")
        return pd.DataFrame([
            {'Symbol': 'AAPL', 'Name': 'Apple Inc.', 'Sector': 'Information Technology'},
            {'Symbol': 'MSFT', 'Name': 'Microsoft Corporation', 'Sector': 'Information Technology'},
            {'Symbol': 'GOOGL', 'Name': 'Alphabet Inc.', 'Sector': 'Communication Services'},
...
def fetch_nikkei225_data():
    ...
    try:
        ...
        if response.status_code != 200:
            st.warning(f"Failed to fetch Nikkei 225 data. Status code: {response.status_code}. Using fallback data.")
            return pd.DataFrame([
                {'Symbol': '7203.T', 'Name': 'Toyota Motor Corp', 'Sector': 'Transportation Equipment'},
                {'Symbol': '6758.T', 'Name': 'Sony Group Corp', 'Sector': 'Electric Appliances'},
                {'Symbol': '9984.T', 'Name': 'SoftBank Group Corp', 'Sector': 'Information & Communication'},
                {'Symbol': '6861.T', 'Name': 'Keyence Corp', 'Sector': 'Electric Appliances'},
                {'Symbol': '8306.T', 'Name': 'Mitsubishi UFJ Financial Group', 'Sector': 'Banks'}
            ])
        ...
        if not company_codes:
            st.warning("No Nikkei 225 data found. Using fallback data.")
            return pd.DataFrame([
                {'Symbol': '7203.T', 'Name': 'Toyota Motor Corp', 'Sector': 'Transportation Equipment'},
                {'Symbol': '6758.T', 'Name': 'Sony Group Corp', 'Sector': 'Electric Appliances'},
                {'Symbol': '9984.T', 'Name': 'SoftBank Group Corp', 'Sector': 'Information & Communication'},
        ...
        st.error(f"Error fetching Nikkei 225 data: {str(e)}. Using fallback data.")
        return pd.DataFrame([
            {'Symbol': '7203.T', 'Name': 'Toyota Motor Corp', 'Sector': 'Transportation Equipment'},
            {'Symbol': '6758.T', 'Name': 'Sony Group Corp', 'Sector': 'Electric Appliances'},
            {'Symbol': '9984.T', 'Name': 'SoftBank Group Corp', 'Sector': 'Information & Communication'},
...
class MarketPredictor:
    def __init__(self, prediction_days=90):
